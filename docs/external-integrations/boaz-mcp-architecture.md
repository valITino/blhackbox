...
blhackbox now integrates BOAZ as a default MCP service under `boaz-mcp/` by cloning the upstream `BOAZ-MCP_gamma` server unchanged and exposing it through a small SSE entrypoint. Claude Code Docker reaches it directly as `boaz` at `http://boaz-mcp:9005/sse`; the MCP Gateway is only an optional host-client aggregation path.
