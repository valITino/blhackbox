...
**Reconnaissance:** `subfinder`, `amass`, `fierce`, `dnsenum`, `whois`
**Scanning:** `nmap`, `masscan`
**Web Testing:** `nikto`, `gobuster`, `dirb`, `whatweb`, `wafw00f`, `wpscan`
**Exploitation:** `sqlmap`, `hydra`

### 2. HexStrike REST API (HTTP, port 8888)
...
### 3. Ollama MCP Server (SSE, port 9000)
...
3. **Kali MCP** — `subfinder -d [TARGET] -silent -all` — subdomain discovery
4. **Kali MCP** — `amass enum -passive -d [TARGET]` — deep passive enumeration
5. **HexStrike** — `POST /api/agents/osint/run` — OSINT intelligence
6. **HexStrike** — `POST /api/intelligence/analyze-target` — AI-driven analysis
...
5. **HexStrike** — `POST /api/agents/network_scan/run` — comprehensive network scan
6. **HexStrike** — `POST /api/agents/vuln_scan/run` — automated vuln detection

For each discovered subdomain with web services:
7. **Kali MCP** — `nmap -sV -T4 --top-ports 100 [SUBDOMAIN]`
...
3. **Kali MCP** — `wpscan --url [TARGET] --enumerate vp,vt,u,dbe` (if WordPress)
4. **HexStrike** — `POST /api/agents/web_recon/run`
5. **HexStrike** — `POST /api/agents/bug_bounty/run`

**NSE Vulnerability Scripts:**
6. **Kali MCP** — `nmap --script=vuln -p [PORTS] [TARGET]`
7. **Kali MCP** — `nmap --script=http-sql-injection,http-vuln* -p 80,443 [TARGET]`
...
5. Test discovered default/weak credentials against login panels
...
9. Test for SSRF via parameter manipulation
10. Test for command injection in input fields
11. Test for LFI/RFI via path traversal patterns
...
1. Collect ALL raw outputs from Phases 1-5 into a single dict:
   ```python
   raw_outputs = {
       "whois": "...", "dnsenum": "...", "subfinder": "...",
       "amass": "...", "nmap_full": "...", "masscan": "...",
       "whatweb": "...", "wafw00f": "...", "nikto": "...",
       "gobuster": "...", "sqlmap": "...", "hydra": "...",
       "nmap_vuln": "...", "wpscan": "...",
